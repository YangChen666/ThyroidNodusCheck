...
/*	函数区*/
	...
	function leftMove(e){
		...
		if(x > getPosition(rightDiv).left-5){
			x = getPosition(rightDiv).left -5;
		}
	...
	function upMove(e){
		...
		if(y > getPosition(downDiv).top-5){
			y = getPosition(downDiv).top -5;
		}
	...
	function rightMove(e){
		...
		if(x < getPosition(leftDiv).left){
			x = getPosition(leftDiv).left;
		}
		var width = mainDiv.offsetWidth - 2;//选择层宽度
		var mainX = getPosition(leftUpDiv).left + 4;//左上角横坐标
	...
	function setChoice(){
		...
		var right = mainDiv.offsetLeft + mainDiv.offsetWidth-2;
		var bottom = mainDiv.offsetTop + mainDiv.offsetHeight-2;
	...
	function preview(view){
		...
		var parentHeight=boxDiv.offsetHeight;
		var parentWidth=boxDiv.offsetWidth;
		strPixel=""+parentHeight+"_"+parentWidth+"_"+view.top+"_"+view.bottom+"_"+view.left+"_"+view.right;
		$("#show").text(strPixel);
		$("#test").val(strPixel);
	}
}